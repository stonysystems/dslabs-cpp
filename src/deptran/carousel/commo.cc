...
void CarouselCommo::BroadcastReadAndPrepare(
  ...
  for (auto& pair : rpc_par_proxies_[par_id]) {
    ...
    auto proxy = (CarouselProxy*)pair.second;
    Future::safe_release(proxy->async_CarouselReadAndPrepare(cmd_id, md, leader, fu2));
  }
}
...
void CarouselCommo::BroadcastDecide(parid_t par_id,
  ...
  Log_debug("send dispatch to site %ld",
            pair_leader_proxy.first);
  auto proxy = (CarouselProxy*) pair_leader_proxy.second;
  FutureAttr fuattr;
