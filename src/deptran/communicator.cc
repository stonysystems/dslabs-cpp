...
void Communicator::BroadcastDispatch(
  ...
  DepId dep_id{"", 0}; // Empty DepId for compatibility
  auto future = proxy->async_Dispatch(cmd_id, dep_id, md, fuattr);
  ...
  for (auto& pair : rpc_par_proxies_[par_id]) {
    if (pair.first != pair_leader_proxy.first) {
      ...
      Future::safe_release(pair.second->async_Dispatch(cmd_id, dep_id, md, fuattr));
    }
  }
}
...
void Communicator::SendPrepare(groupid_t gid,
  ...
  Log_debug("SendPrepare to %ld sites gid:%ld, tid:%ld\n",
            ...
            tid);
  DepId dep_id{"", 0}; // Empty DepId for compatibility
  Future::safe_release(proxy->async_Prepare(tid, sids, dep_id, fuattr));
}
...
void Communicator::SendCommit(parid_t pid,
  ...
  DepId dep_id{"", 0}; // Empty DepId for compatibility
  Future::safe_release(proxy->async_Commit(tid, dep_id, fuattr));
}
...
void Communicator::SendAbort(parid_t pid, txnid_t tid,
  ...
  DepId dep_id{"", 0}; // Empty DepId for compatibility
  Future::safe_release(proxy->async_Abort(tid, dep_id, fuattr));
}
...
void Communicator::SendEarlyAbort(parid_t pid, txnid_t tid) {
  ClassicProxy* proxy = LeaderProxyForPartition(pid).second;
  FutureAttr fuattr;
  fuattr.callback = [](Future*) {};
