...
void ServerWorker::SetupBase() {
  ...
  if (yaml["lab"].IsDefined() && yaml["lab"]["shard"].IsDefined() &&
    ...
    if (this->site_info_->partition_id_ == 0) {
      ...
      rep_log_svr_->app_next_ = [this, sk_svr](int slot, shared_ptr<Marshallable> m) -> int {
        if (!m) return 0;

        auto cmd = dynamic_cast<MultiStringMarshallable*>(m.get());
        if (cmd && cmd->data_.size() >= 2) {
          const auto& op = cmd->data_.at(1);
          if (op == "join" || op == "leave" || op == "move" || op == "query") {
            sm_svr_->OnNextCommand(*m);
            return 0;
          }
          if (op == "put" || op == "append" || op == "get") {
            sk_svr->OnNextCommand(*m);
            return 0;
          }
        }

        Log_error("unknown shard lab command committed at slot %d", slot);
        return 0;
      };
    } else {
...
void ServerWorker::PopTable() {
  ...
  if (sharding_->tb_infos_.size() == 0) {
    verify(Config::GetConfig()->benchmark() == BENCH_NONE);
    return;
  }
...
void ServerWorker::RegisterWorkload() {
  if (Config::GetConfig()->benchmark() == BENCH_NONE) {
    return;
  }
...
void ServerWorker::SetupCommo() {
  ...
  if (rep_sched_ && rep_sched_->site_id_ == 0) {
    ...
    auto all_sites_ready = []() -> bool {
      std::lock_guard<std::recursive_mutex> lk(RaftFrame::raft_test_mutex_);
      if (!RaftFrame::all_sites_created_s) return false;
      if (RaftFrame::frames_.empty()) return false;
      for (const auto& entry : RaftFrame::frames_) {
        auto* frame = entry.second;
        if (!frame || !frame->svr_ || !frame->commo_ || !frame->svr_->setup_done_) {
          return false;
        }
      }
      return true;
    ...
    while (!all_sites_ready()) {
      std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
    // Let startup traffic settle before measurements in test 9.
    std::this_thread::sleep_for(std::chrono::milliseconds(300));
    Log_info("SetupCommo: all raft test sites ready, launching test thread");
    ...
    std::thread test_thread([this]() {
      ...
      Coroutine::CreateRun([this]() {
        ...
        if (raft_frame) {
          Log_info("Test: frame kv/sm=%p/%p, raft_frame kv/sm=%p/%p",
                   rep_frame_->kv_svr_, rep_frame_->sm_svr_,
                   raft_frame->kv_svr_, raft_frame->sm_svr_);
        }
        // Prefer base Frame pointers to avoid shadowed member state in RaftFrame.
        test.kv_svr_ = rep_frame_->kv_svr_;
        test.sm_svr_ = rep_frame_->sm_svr_;
        if (!test.kv_svr_ && raft_frame) test.kv_svr_ = raft_frame->kv_svr_;
        if (!test.sm_svr_ && raft_frame) test.sm_svr_ = raft_frame->sm_svr_;
