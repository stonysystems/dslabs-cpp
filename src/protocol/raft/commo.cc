...
RaftCommo::SendAppendEntries2(siteid_t site_id,
  ...
  auto ret = Reactor::CreateSpEvent<IntEvent>();
    // Log_info("reached commo.cc SendAppendEntries2");
...
RaftCommo::BroadcastVote(parid_t par_id,
                         ...
                         siteid_t self_id,
                         ballot_t cur_term ) {
  //Log_info("reached broadcast event in commo.cc");
...
//  Log_info("about to access rpc_par_proxies_ for par_id: %d, this ptr: %p", par_id, this);
  //Log_info("rpc_par_proxies_ size: %zu", rpc_par_proxies_.size());
  auto proxies = rpc_par_proxies_[par_id];
   // Log_info("reached broadcast event in commo.cc, proxies size: %zu", proxies.size());
