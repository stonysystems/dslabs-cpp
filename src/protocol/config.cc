...
//  std::string filename = "./config/sample.yml";
  ...
  optind = 1;
  string filename;
  while ((c = getopt(argc, argv, "bc:d:f:h:i:k:p:P:r:s:S:t:H:T:n:D:A:")) != -1) {
    switch (c) {
...
void Config::Load() {
  Log_info("Config::Load: config_paths_ size=%d", config_paths_.size());
  for (auto &name: config_paths_) {
    Log_info("Config::Load: loading config file: %s", name.c_str());
  ...
  Log_info("Config::Load: after loading all configs, sites_ size=%d", sites_.size());

  // TODO particular configuration for certain workloads.
  this->InitTPCCD();

  // Additional checking for non-benchmark runs
  verify((benchmark_ != BENCH_NONE) == (NumClients() > 0));
  if (!sharding_) {
    verify(benchmark_ == BENCH_NONE);
...
void Config::LoadYML(std::string &filename) {
  ...
  Log_info("LoadYML: config has 'process': %d, has 'site': %d",
           config["process"].IsDefined(), config["site"].IsDefined());

  if (config["process"]) {
    BuildSiteProcMap(config["process"]);
    Log_info("BuildSiteProcMap: site_proc_map_ size=%d", site_proc_map_.size());
  }
  if (config["site"]) {
    LoadSiteYML(config["site"]);
    Log_info("LoadSiteYML: sites_ size=%d", sites_.size());
  }
  if (config["process"]) {
...
void Config::InitBench(std::string &bench_str) {
  if (bench_str.empty() || bench_str == "none") {
    benchmark_ = BENCH_NONE;
    return;
  }
  Log_error("Unsupported workload '%s': src/bench has been removed",
            bench_str.c_str());
  verify(0);
}
...
void Config::InitTPCCD() {
  // No-op in lab-only tree; TPCC-specific setup was part of src/bench.
}
...
Config::SitesByProcessName(string proc_name, Config::SiteInfoType type) {
  ...
  Log_info("SitesByProcessName: looking for proc_name='%s', type=%d, total sites=%d",
           proc_name.c_str(), type, searching->size());
  ...
  std::for_each(searching->begin(), searching->end(),
                [proc_name, &result](SiteInfo& site) mutable {
                  ...
                  Log_info("  Checking site name='%s' proc_name='%s'",
                           site.name.c_str(), site.proc_name.c_str());
                  if (site.proc_name==proc_name) {
                    result.push_back(site);
                  }
                });
  Log_info("SitesByProcessName: found %d sites", result.size());
  return result;
}
