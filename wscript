...
#    _enable_snappy(conf)
    ...
    # Add RustyCpp include path (needed for RRR framework migration)
    conf.env.append_value('INCLUDES', ['third-party/rusty-cpp/include'])
...
#            "bin/rpcgen rlog/log_service.rpc")
    ...
    _depend("src/protocol/rcc_rpc.h src/protocol/rcc_rpc.py",
            "src/protocol/rcc_rpc.rpc",
            "bin/rpcgen --python --cpp src/protocol/rcc_rpc.rpc")
...
#              use="base PTHREAD")
    ...
    bld.stlib(source=bld.path.ant_glob("src/memdb/*.cc"), target="memdb",
              includes="src src/rrr src/protocol src/base third-party/rusty-cpp/include",
              use="rrr PTHREAD")
    ...
    bld.objects(source=bld.path.ant_glob("src/protocol/*.cc "
                                       "src/protocol/*/*.cc "
                                       "src/bench/*/*.cc",
                                       excl=['src/protocol/s_main.cc', 'src/protocol/paxos_main_helper.cc','src/protocol/lab_solution_raft/*.cc', 'src/bench/paxos_lib/network_bench.cc']),
              target="deptran_objects",
              includes="src src/rrr src/protocol third-party/rusty-cpp/include",
              uselib="YAML-CPP BOOST",
              use="externc rrr memdb PTHREAD PROFILER RT")

    bld.shlib(source=bld.path.ant_glob("src/protocol/paxos_main_helper.cc "),
              target="txlog",
              includes="src src/rrr src/protocol third-party/rusty-cpp/include",
              uselib="YAML-CPP BOOST",
              use="externc rrr memdb deptran_objects PTHREAD PROFILER RT")

    bld.program(source=bld.path.ant_glob("src/protocol/s_main.cc"),
    ...
    #                                     "src/protocol/paxos_main_helper.cc"),
    #            target="microbench",
    #            includes="src src/rrr src/protocol ",
    #            uselib="YAML-CPP BOOST",
    #            use="externc rrr memdb deptran_objects PTHREAD PROFILER RT")
...
def _enable_rust_implementation(conf):
    if Options.options.use_rust_impl:
        Logs.pprint("PINK", "Rust implementation of RaftServer enabled")

        # Check if cargo is available
        conf.find_program('cargo', var='CARGO', mandatory=True)

        # Add compiler flag to use Rust implementation
        conf.env.append_value("CXXFLAGS", "-DRAFT_USE_RUST")

        # Set environment variable to indicate Rust is enabled
        conf.env.RUST_ENABLED = True

        Logs.pprint("PINK", "Rust implementation will be built and linked")
...
def _gen_srpc_headers():
    for srpc in glob.glob("src/protocol/*/*.rpc"):
