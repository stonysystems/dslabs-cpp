...
def options(opt):
    ...
    opt.add_option('', '--enable-event-timeout', dest='event_timeout',
                   default=False, action='store_true')
    opt.add_option('', '--protocol', dest='protocol',
                   default='raft', action='store')
...
#            "bin/rpcgen rlog/log_service.rpc")
    ...
    # Helloworld RPC generation - DISABLED (not needed)
...
#              use="base PTHREAD")
    ...
    for protocol in Options.options.protocol.split(','):
        dirs += "src/deptran/{}/*.cc ".format(protocol)

    bld.objects(source=bld.path.ant_glob(dirs,
                    excl=['src/deptran/s_main.cc', 
                        'src/deptran/paxos_main_helper.cc', "src/bench/paxos_lib/*"]),
...
#              use="externc rrr memdb deptran_objects PTHREAD PROFILER RT")
    ...
    # Microbench - DISABLED (test program with missing dependencies)
    ...
    # Microbench paxos network - DISABLED (test program with missing network client dependencies)
    ...
    # NC main - DISABLED (test program with missing network client dependencies)
    ...
    # simple rpc, helloworld - DISABLED (example program with missing dependencies)
