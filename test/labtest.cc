#include "labtest.h"
#include "../src/kv/server.h"
#include "../src/kv/client.h"
#include "../src/shardkv/client.h"
#include "../src/shardkv/server.h"
#include "../src/shardmaster/service.h"
#include "../src/shardmaster/client.h"
...
int RaftLabTest::Run(void) {
  ...
  if (config_node) {
    if (config_node["shard"].IsDefined() && config_node["shard"].as<bool>()) {
      return RunShard();
    }
    if (config_node["kv"].IsDefined() && config_node["kv"].as<bool>()) {
      return RunKv();
    }
    if (config_node["raft"].IsDefined() && config_node["raft"].as<bool>()) {
      return RunRaft();
    }
  } else {
    verify(0);
...
int RaftLabTest::testBackup(void) {
  ...
  AssertOneLeader(leader1);
  Log_debug("%d is the leader, disconnect 3 followers: %d, %d, %d", leader1, (leader1 + 2) % NSERVERS, (leader1 + 3) % NSERVERS, (leader1 + 4) % NSERVERS);
  ...
  Log_debug("disconnect the leader %d, and its 1 follower, reconnect the 3 followers: %d, %d, %d", leader1, (leader1 + 2) % NSERVERS, (leader1 + 3) % NSERVERS, (leader1 + 4) % NSERVERS);
  ...
  Log_debug("reconnect the old leader %d and the follower %d", leader1, (leader1 + 1) % NSERVERS);
  ...
  Log_debug("check if the old leader has enough committed at index %ld", index);
  int nc = config_->NCommitted(index);
  Log_debug("Total servers committed at index %ld: %d (expected %d)", index, nc, NSERVERS);
...
int RaftLabTest::testCount(void) {
  ...
  const uint64_t max_init_rpcs = 12 * NSERVERS;
  Assert2(init_rpcs_ > 1 && init_rpcs_ <= max_init_rpcs,
          "too many or too few RPCs (%ld) to elect initial leader (max %ld)",
          init_rpcs_, max_init_rpcs);
...
int RaftLabTest::testUnreliableAgree(void) {
  ...
  Log_debug("TEST10: After Init2, about to SetUnreliable(true)");
  config_->SetUnreliable(true);
  Log_debug("TEST10: After SetUnreliable(true), starting test loop");
  std::vector<uint64_t> retvals{};
  std::mutex mtx{};
  for (int iter = 1; iter < 50; iter++) {
    Log_debug("TEST10: Starting iteration %d", iter);
    // Create threads for this iteration only
    std::vector<pthread_t> iter_threads{};
    for (int i = 0; i < 4; i++) {
      ...
      verify(pthread_create(&thread,
                            ...
                            (void*)args) == 0);
      iter_threads.push_back(thread);
    }
    Log_debug("TEST10: Created 4 threads for iteration %d", iter);
    if (retvals.size() > 0)
      break;
    Log_debug("TEST10: About to call DoAgreement for iteration %d", iter);
    auto result = config_->DoAgreement(1000 + iter, 1, true);
    Log_debug("TEST10: DoAgreement returned %ld for iteration %d", result, iter);
    if (result == 0) {
      std::lock_guard<std::mutex> lock(mtx);
      retvals.push_back(0);
      Log_debug("TEST10: Breaking due to failure in iteration %d", iter);
      // Join threads from this iteration before breaking
      for (auto thread : iter_threads) {
        pthread_join(thread, nullptr);
      }
      break;
    }
    // Join threads from this iteration before moving to next
    Log_debug("TEST10: Joining 4 threads from iteration %d", iter);
    for (auto thread : iter_threads) {
      verify(pthread_join(thread, nullptr) == 0);
    }
    Log_debug("TEST10: Completed iteration %d", iter);
  }
  Log_debug("TEST10: Loop completed, turning off unreliable mode");
  config_->SetUnreliable(false);
  Log_debug("TEST10: Unreliable mode off");
